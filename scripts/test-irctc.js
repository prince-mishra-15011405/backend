/**
 * Standalone IRCTC Air Scraper Verification Script
 * Tests the IRCTC Air API scraper directly and displays normalized observations in standard schema.
 * 
 * Usage:
 *   node scripts/test-irctc.js [origin] [destination] [departureDate] [days]
 *   node scripts/test-irctc.js BOM MAA 2026-09-05 1
 *   node scripts/test-irctc.js DEL BOM 2026-09-05 3
 *   node scripts/test-irctc.js --all
 */

require("dotenv").config();
const irctcScraper = require("../scrapers/irctc_scraper");
...
async function scrapeSingleRoute(origin, destination, departureDate, days = 1) {
  const job = {
    ...
    departureDate: departureDate || "2026-09-05",
    days: parseInt(days || 1, 10)
  };

  console.log(`\n========================================`);
  console.log(`TESTING IRCTC AIR SCRAPER: ${job.origin} → ${job.destination}`);
  console.log(`Target Date: ${job.departureDate} (${job.days} day(s))`);
  console.log(`========================================`);

  try {
    const startTime = Date.now();
    const observations = await irctcScraper.scrape(job);
    ...
    if (observations.length > 0) {
      ...
      const rfsDoc = irctcScraper.toRouteFareSearchDocument(
        observations,
        job.origin,
        job.destination,
        job.departureDate
      ...
      if (isDbConnected()) {
        ...
        await scraperService.recordHistoricalFare(`${job.origin}-${job.destination}`, observations, "IRCTC Air");
      }
...
async function run() {
  ...
  if (args.includes("--all") || args.includes("-a")) {
    console.log("========================================");
    console.log("MULTI-ROUTE IRCTC AIR BATCH VERIFICATION");
    console.log("========================================");

    const testRoutes = [
      { origin: "BOM", destination: "MAA", date: "2026-09-05", days: 1 },
      { origin: "DEL", destination: "BOM", date: "2026-09-05", days: 1 },
      { origin: "DEL", destination: "BLR", date: "2026-09-05", days: 1 }
    ];
    ...
    const destination = args[1] || "MAA";
    const departureDate = args[2] || "2026-09-05";
